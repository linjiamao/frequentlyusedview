...
public class MySlideView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < MainActivity.pinyinList.size(); i++) {
            String textView = MainActivity.pinyinList.get(i);
            if (i == position - 1) {
                paint.setColor(getResources().getColor(R.color.error_color));
                selectTxt = MainActivity.pinyinList.get(i);
            ...
            mTextHeight += mHeight / MainActivity.pinyinList.size();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                mTextHeight = mHeight / MainActivity.pinyinList.size();
                position = y / (mHeight / (MainActivity.pinyinList.size() + 1));
                invalidate();
                break;
            case MotionEvent.ACTION_MOVE:
                if (isSlide) {
                    backgroundColor = getResources().getColor(R.color.font_text);
                    mTextHeight = mHeight / MainActivity.pinyinList.size();
                    position = y / (mHeight / MainActivity.pinyinList.size() + 1) + 1;
                    invalidate();
                }
                break;
            case MotionEvent.ACTION_UP:
